...
X, y = cc_apps.iloc[:, 0:20], cc_apps.iloc[:, 20]
X= X.values
y= y.values
...
## 9. Fitting a random forest model to the train set
...
#model = joblib.load('Decision_tree_model.pkl')  # 'Decision_tree_model.pkl' trained model file
...
def preprocess_input(user_input):
    ...
    user_df[numeric_cols.columns] = user_df[numeric_cols.columns].fillna(user_df[numeric_cols.columns].mean())
...
def predict_credit_approval(user_input):
    ...
    Returns:
    - int: 1 if approved, 0 if not approved.
    """
    ...
    return prediction[0]  # Assuming your model returns an array; adjust accordingly
